...
main () {
  ...
  do
    ...
    then
      echo "$i is not empty"
      if [[ $i == $GLUSTERFS_META_DIR ]]
      then
        echo "copying groups and hooks dir from ${i}_bkp to $i" > ${GLUSTERFS_LOG_CONT_DIR}/${i_mangled}_ls
        cp -r ${i}_bkp/hooks/* ${i}/hooks/
        cp -r ${i}_bkp/groups/* ${i}/groups
        ls -R $i >> ${GLUSTERFS_LOG_CONT_DIR}/${i_mangled}_ls
      fi
    else
      bkp=$i"_bkp"
      cp -r $bkp/* $i
